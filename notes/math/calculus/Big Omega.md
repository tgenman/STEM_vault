...
START
Math_TWO_side
FRONT: Функция $f(n)$ является $\Omega(g(n))$
Асимптотическая оценка снизу
BACK: если существуют такие положительные константы $C$ и $n_0$, что для всех $n \geq n_0$ выполняется неравенство $|f(n)| \geq C \cdot |g(n)|$.
